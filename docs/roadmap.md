...
| GAP-025 | `scripts/build-digests.ps1` reads markers line by line and honours fenced blocks but not inline code spans, so a document that *mentions* `<!--` inside backticks opens a comment as far as the parser is concerned and silently swallows the next marker it meets. Generator and `-Check` agree with each other because they share the parser, so CI is green and the digest is simply one rule short — the failure has no reader. Measured while writing feature 014's phase 4 prose: 79 markers harvested where 80 existed, restored only by moving the syntax example into a fenced block. This is `enforcement-pack.ps1`'s B7 defect in a second tool, and that one was found the same way: a document that discusses comment syntax is the document most likely to be about the check that reads it. Candidate fix: reuse the CommonMark-shaped visibility logic feature 014 built for the amendment check (fences by opener run length, inline spans by equal-length backtick pairing, `\`` delimiting nothing) rather than writing a third line-based parser | 2026-09-16 kit feature 014 phase 4, writing the adoption prose |
| GAP-026 | `Get-Territory` (`scripts/scope-lib.ps1`) matches `**Territory**:` at line start only, so a decorated marker — `**Territory** (widened by amendment …):`, the form an author naturally writes when annotating *why* the block changed — declares nothing the parser can see. The phase then degrades to a non-blocking `WARN … pre-006 compatibility`, which is the wrong excuse in a `tasks.md` whose sibling phases all declare territory properly, and the phase commit is never graded at all. Observed twice on one branch in feature 014: once from the decorated marker itself, and once from a repair commit graded against a parent that still carried it. Both were caught by hand, not by the check, and in both cases a phase commit could have touched anything. Two candidate fixes, and the second matters more than the first: accept the decorated spelling, **and** make an undeclared territory FAIL rather than WARN when any other phase in the same file declares one — the pre-006 excuse is checkable, and while it is merely asserted the check cannot tell a legacy file from a typo | 2026-09-16 kit feature 014 phase 4, remediating the phase's own scope check |
| GAP-027 | An `enforcement-pack.ps1` run that grades **nothing** still reports `OK` and exits 0. Feature 014 phase 5 taught the amendment check to fail loudly when it cannot read history, but the other members that take a diff base return without grading when no base is computable — the ordinary depth-1 `actions/checkout` shape on a branch whose base is absent — and the run's verdict block, its exit code and the GitHub check conclusion all read exactly as they read on a fully graded run. The member that goes quiet includes `Invoke-ReviewProvenanceCheck`, the machine half of gate 5 itself: on such a clone a review file with no Reviewer Provenance block passes green. Phase 5 made this **audible** — one warning line in the log body — and deliberately went no further, because FR-009 forbids a new hard failure on the Lite lane; nothing yet makes it **visible** where a required-status badge or a reviewer reading a verdict block would see it. Pre-existing and measured identical before phase 5. Candidate fix: a distinct verdict state (`enforcement-pack: UNGRADED`) that is not `OK`, separate from the exit code, so the Lite lane keeps exit 0 while no one can read a quiet run as a clean one; the script's `.DESCRIPTION` header and `adoption/updating.md`'s Lite-lane paragraph both need the same state named, since neither mentions it today | 2026-09-18 kit feature 014 phase 5, round 3 fresh-context review, finding H4 |
| GAP-028 | `Convert-CodeSpanMarkers` (`scripts/markdown-lib.ps1`, shared by `enforcement-pack.ps1` and `build-digests.ps1`) disarms comment markers **one line at a time**, so a CommonMark inline code span whose closing backtick run falls on a later line leaves its `<!--` armed. The next literal `-->` anywhere — plausibly a real digest marker's own closing arrow — shuts the phantom comment and everything between vanishes with no message: GAP-025's exact symptom in the shape GAP-025's fix does not reach. Demonstrated by feature 015's phase 2 fresh-context review on a three-marker document, of which two were harvested. Not a regression — the per-line design is feature 014's reviewed tradeoff, and the function was moved byte-identical — but it means "GAP-025 closed" holds only for the single-line shape. It matters more here than it did in 014: the same function now feeds `Get-VisibleFromText`, so the blind spot sits underneath amendment-authority grading as well as the digests. Candidate fix: carry code-span state across lines within a paragraph the way `Get-FencedLineMap` already carries fence state across the document, and prove it on both consumers before either is trusted | 2026-09-19 kit feature 015 phase 2 fresh-context review, finding F2 |
| GAP-029 | `scripts/territory-check.ps1` speaks none of the kit's verdict vocabulary and cannot be asserted on its own error paths. It runs under `$ErrorActionPreference = 'Stop'` and reports four refusals — outside a git repository, a branch that is not `NNN-name`, a failed `git fetch origin`, a base it cannot diff against — with `Write-Error`, which `Stop` makes terminating: the `exit 1` written after each is unreachable, the exit code a caller sees is PowerShell's rather than the script's, and through the enforcement harness's launcher the run reports exit 97 (*did not run to a verdict*). Those four rules are therefore exempted from fixture coverage (`tests/enforcement/rules.json` TERR-008..011) with this as the written reason. A fifth path is reachable and now fixtured (TERR-012), and the fixture shows a further defect: when a branch in the claim ledger cannot be compared (no remote-tracking ref — any single-branch fetch refspec), the script prints a warning, **skips it, and still ends on `CLEAN`** — a claim that nothing overlaps, made over a comparison it did not run. That is GAP-027's species in the pre-phase check: it should say `UNGRADED` for the skipped branch, or refuse. Nothing is broken for a human at a terminal today (`pwsh -File` still exits 1 on the refusals); what is missing is a script that grades in the kit's own words and can be proved to. | 2026-09-26 kit feature 015 phase 6 fresh-context review, findings F3 and F8 |
| GAP-030 | `scripts/scope-check-repos.ps1` summarises a **partly** graded run as if it were whole. The run-level `UNGRADED` line (`:415`) is printed only when nothing at all was graded, so when one declared code repository is graded and another is not — it prints its own `<repo>: UNGRADED …`, or, when every commit it offers is skipped, only `not applicable` / `WARN` lines — the script prints no run-level verdict, exits 0, and `ritual-checks.ps1` (which matches `^scope-repos: UNGRADED`) reports `scope-repos OK`. That is GAP-027's species one level down: a summary word over a run part of which compared nothing. Related, same script: the hint printed when no trunk resolves says `pass -BaseRef <ref>`, but `-BaseRef` is read only under `-All` and, without `-Repo`, replaces the trunk candidates for **every** declared repository — the form that works is `-All -Repo <repo> -BaseRef <trunk>`, and `ritual-checks.ps1` does not pass `-BaseRef` through at all. Both are disclosed to adopters in the 015 flow-down note (`adoption/updating.md`) and in `docs/sdlc/review-process.md` step 3 (read the per-repository lines, not the summary). | 2026-09-26 kit feature 015 phase 6 fresh-context review, round 4 F2 and round 5 F1/F2 |
...
| Multi-line code spans (`Convert-CodeSpanMarkers` carries inline code-span state across the lines of a paragraph, the way `Get-FencedLineMap` carries fence state across a document, so a span whose closing backtick run falls on a later line can no longer leave a comment marker armed; proven with fixtures on both consumers, the digest harvest and amendment-authority grading, before either is trusted) | GAP-028 | P1 | in progress | anas.m | `[specs/016-multiline-code-spans/]` |
...
- 2026-09-16 GAP-025 and GAP-026 recorded from kit feature 014's own phase 4, and
  deliberately **not** absorbed into it. Both are fail-opens in the kit's tooling and both
  were found by the same feature, which makes absorbing them tempting: the branch was open,
  the evidence was in hand, and the work is small. `specs/014-amendment-authority/spec.md`
  forbids it in as many words — "Both are separate rows; this feature must not quietly absorb
  them" — and the code belongs to features 010 and 006/012, not to amendment authority. The
  third fail-open the same review found *was* taken into 014 as phase 5, because it was that
  feature's own check failing to grade: `enforcement-pack.ps1` returned bare on a shallow
  clone, an unresolvable base and an unreadable parent, so a green run proved nothing. The
  line between the three is ownership, not convenience.
  Worth stating plainly, because it is the argument for both rows: all three defects pass
  quietly where they should speak, and a kit whose own feature 014 argues that a check failing
  open is worse than a noisy one should not keep two of them in a drawer. Neither is claimed.
  Recommended at P2 — neither blocks a live feature, and both are cheap next to what they
  prevent.
- 2026-09-18 GAP-027 recorded from feature 014 phase 5's third review round, and left out of
  014 on the same ownership line the 2026-09-16 entry drew. Phase 5 closed the amendment
  check's own fail-open; H4 is the *rest* of the pack behaving the way the amendment check
  used to, plus a verdict vocabulary that has no word for "graded nothing". Absorbing it
  would mean editing members feature 014 has no claim on and, worse, changing what a green
  `enforcement-pack` line means for every adopted project — a change that deserves its own
  spec, not a remediation round. Recommended at P2 with one argument for P1: the quiet
...
- 2026-09-19 **GAP-028 recorded from feature 015's own phase 2 review**, and deliberately NOT folded into 015. The multi-line code-span blind spot reaches `enforcement-pack.ps1`'s amendment grading through the shared function, so it needs its own demonstration and its own review rather than a tail-end edit to a phase already reviewed. The same review's F3 — a `**Territory**` marker whose annotation wraps across lines matches no anchor — was closed inside 015 instead, by making the near-miss loud rather than by adding a multi-line scanner
...
- 2026-09-27 **GAP-028 promoted to a roadmap row and claimed in the same sitting** as feature 016, at P1, owner anas.m, who chose it as the next feature after 015's flow-down. P1 because the blind spot sits under amendment-authority grading, not only under the digests. Promotion and claim share one commit: a row created at `idea` would read as unstarted while the branch is already live, which is exactly what `scripts/roadmap-claim-check.ps1` fails.
