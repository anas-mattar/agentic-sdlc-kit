...
```
   ...
   neither verdict may be FAIL — `n/a`, `not applicable`, `WARN` and `UNGRADED` are the
   cross-repo check's lawful non-blocking verdicts, and a phase that touches no code repository
   legitimately produces one.

   **`UNGRADED` is not one of the acceptable ones in the same sense.** It exits 0, so it will
   not stop you, and it means the check RAN AND COMPARED NOTHING: no diff base, an unreadable
   parent, a clone without the history it needs. A reviewer who accepts a phase on an
