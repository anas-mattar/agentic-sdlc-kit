...
- `{{BACKEND_REPO}}/src/Training/**`
- `{{FRONTEND_REPO}}/app/(training)/**`
- `specs/NNN-name/`
```
...
array, and reports `n/a` when none is declared or none is present — so single-repo
projects and governance-only checkouts are unaffected. Two rules make it work:
